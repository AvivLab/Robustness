...
legend(h1, 'final population robustness','initial population robustness','Location','southeast')
xlabel(h1, 'viable probability');
...
title(h3,"Genetic (12,0.2)");
xlabel(h3,"Number of neighbors");
...
plot(h2,mus(1:13),hipstruct.sls(1:13,:));
...
figure(); %figure 5
h1=subplot(1,3,1);
plot(h1,mus,lowpstruct.ds);

h2=subplot(1,3,2);
plot(h2,mus, hipstruct.ds);
...
figure(); %figure 6
h1=subplot(1,3,1);
plot(h1,mus,lowpstruct.avgeigs);
hold on
...
figure(); %(figure 7) Figure S2: Eigencentrality of the mode, normalized so that 1 is the maximum possible value.
h1=subplot(1,3,1);
...
figure(); hold on
plot(-log(mus(1:13)),mean(isopeakfreqs(:,1:13))',"-*b");
plot([-log(0.004),10],mean(isopeakfreqs(:,13:14)),"--*b");
%%unifpeak
unifpeakfreqs=getPeakFreq(lowpstruct.indss,lowpstruct.pss,10);
plot(-log(mus(1:13)),mean(unifpeakfreqs(:,1:13))',"-*m");
plot([-log(0.004),10],mean(unifpeakfreqs(:,13:14)),"--*m");
%%isomax
isomaxfreqs=getMaxFreqs(lowpstruct.indss,lowpstruct.isopss,10);
plot(-log(mus(1:13)),mean(isomaxfreqs(:,1:13))',"-*r");
plot([-log(0.004),10],mean(isomaxfreqs(:,13:14)),"--*r");
%%unifmax
unifmaxfreqs=getMaxFreqs(lowpstruct.indss,lowpstruct.pss,10);
plot(-log(mus(1:13)),mean(unifmaxfreqs(:,1:13))',"-*g");
plot([-log(0.004),10],mean(unifmaxfreqs(:,13:14)),"--*g");
%%plot settings
